...
const FallbackComponent: React.FC<FallbackProps> = ({ error, resetErrorBoundary }) => {
  return (
    <Card className={cn('m-5 p-8 text-center text-destructive')}>
      ...
      <p className={cn('mb-4')}>
        The most likely cause for an error like this is malformed input data. Please ensure that the input files are
        valid and readable and then try again. If the error persists, please report it including the below stack trace.
      </p>
      <p className={cn('mb-8')}>
