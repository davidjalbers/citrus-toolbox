...
export function useMultistepForm<DataTuple extends unknown[]>(
  addSteps: (form: MultistepForm<[]>) => MultistepForm<DataTuple>,
) {
  ...
  return {
    viewSteps: viewSteps.map((step, idx) => ({
      ...step,
      status: viewStepStatus[idx],
    })),
    viewStepIndex,
