...
import { JobResult, JobArg, JobArgSchema, PrivacyFormEntry, PrivacyFormEntrySchema, SurveyEntry, SurveyEntrySchema, Status } from "./core";
...
export async function writeJobResultToCsv(arg: {
  ...
  const wsAllStudyCodes = csvAllStudyCodes.pipe(fs.createWriteStream(path.join(outputDirectoryPath, 'StudyCodes_all.csv')));
  const wsValidStudyCodes = csvValidStudyCodes.pipe(fs.createWriteStream(path.join(outputDirectoryPath, 'StudyCodes_valid.csv')));
  const wsCommentedPrivacyForm = csvCommentedPrivacyForm.pipe(fs.createWriteStream(path.join(outputDirectoryPath, `${path.basename(privacyFormFilePath, '.csv')}_commented.csv`)));
  const wsCommentedSurvey = csvCommentedSurvey.pipe(fs.createWriteStream(path.join(outputDirectoryPath, `${path.basename(surveyFilePath, '.csv')}_commented.csv`)));

  const getStatusVisualization = (status: Status): string => {
    switch (status) {
      case 'OK_VALID':
      case 'ERROR_NO_CONSENT': return 'P+S';
      case 'ERROR_ONLY_PRIVACY_FORM': return 'P';
      case 'ERROR_ONLY_SURVEY': return 'S';
      case 'ERROR_INVALID': return '';
    }
  };
